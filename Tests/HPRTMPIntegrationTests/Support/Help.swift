...
struct AudioSpecificConfig {
  ...
  init(data: Data) {
    ...
    self.frameLengthFlag = UInt8(data[1] & 0b00100000) == 1
    self.dependsOnCoreCoder = data[1] & 0b000000010
    self.extensionFlag = data[1] & 0b000000001
  }

  init(objectType: MPEG4ObjectID, channelConfig: ChannelConfigType, frequencyType: SampleFrequencyType,
       frameLengthFlag: Bool = false, dependsOnCoreCoder: UInt8 = 0, extensionFlag: UInt8 = 0) {
  ...
  var encodeData: Data {
    let flag = frameLengthFlag ? 1 : 0
    let first = UInt8(objectType.rawValue) << 3 | UInt8(frequencyType.rawValue >> 1 & 0b00000111)
    let second = (0b10000000 & frequencyType.rawValue << 7) |
      (0b01111000 & channelConfig.rawValue << 3) |
      (UInt8(flag) << 2) |
      (dependsOnCoreCoder << 1) |
      extensionFlag
