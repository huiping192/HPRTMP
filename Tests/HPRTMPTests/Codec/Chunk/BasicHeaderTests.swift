...
class BasicHeaderTests: XCTestCase {
  func testEncodeForStreamIdLessThanOrEqualTo63() {
    let header = BasicHeader(streamId: ChunkStreamId(63), type: .type0)
  ...
  func testEncodeForStreamIdLessThanOrEqualTo319() {
    let header = BasicHeader(streamId: ChunkStreamId(319), type: .type1)
    let encodedData = header.encode()
  ...
  func testEncodeForStreamIdGreaterThan319() {
    let header = BasicHeader(streamId: ChunkStreamId(320), type: .type2)
    let encodedData = header.encode()
  ...
  func testEncodeForDifferentTypes() {
    let header1 = BasicHeader(streamId: ChunkStreamId(319), type: .type0)
    let encodedData1 = header1.encode()
    
    XCTAssertEqual(encodedData1, Data([(0 << 6) | 0, 255]))
    
    let header2 = BasicHeader(streamId: ChunkStreamId(63), type: .type3)
