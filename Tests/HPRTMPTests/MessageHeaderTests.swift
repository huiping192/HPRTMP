...
class MessageHeaderType0Tests: XCTestCase {
  ...
  func testEncodeWithMaxTimestamp() {
    let header = MessageHeaderType0(timestamp: 16777215,
                                    ...
                                    type: .video,
                                    messageStreamId: 1)
    let expected = Data([0xff, 0xff, 0xff, 0x00, 0x00, 0x05, 0x09, 0x00, 0x00, 0x00, 0x01])
  ...
  func testEncodeWithExtendedTimestamp() {
    let header = MessageHeaderType0(timestamp: 16777216,
                                    ...
                                    type: .video,
                                    messageStreamId: 1)
    let expected = Data([0xff, 0xff, 0xff, 0x00, 0x00, 0x05, 0x09, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00])
    XCTAssertEqual(header.encode(), expected)
  }
  ...
  func testEncodeWithSmallTimestamp() {
    let header = MessageHeaderType0(timestamp: 1,
                                    ...
                                    type: .video,
                                    messageStreamId: 1)
    let expected = Data([0x00, 0x00, 0x01, 0x00, 0x00, 0x05, 0x09, 0x00, 0x00, 0x00, 0x01])
